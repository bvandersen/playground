...
- **Every day is different in mood, not just in mechanic**: each recipe
  has a `register` (see "Registers") and the draw never repeats
  yesterday's register, and over any 7 days shows at least 3 registers
  with no more than 3 eerie days. Registers are balanced in the
  curated first week too (it must include one tender and one teaching).
...
### Sound: synth first, generated assets where synth falls short

(Added from the user's follow-up: "some might have sounds, e.g.
generated by the ElevenLabs MCP".)

- **Default is `Synth`** (Phase 1): drones, sines, noise, chimes — tiny,
  endlessly variable per recipe, no files.
- **Generated assets** for what synthesis does badly: whispers and
  spoken lines, textured foley (stone door grinding, match strike, moss,
  wind in a doorway, a pick on rock), choral fragments, deep-sea groans,
  shortwave voices. Generated with the **ElevenLabs MCP** tools (speech,
  sound effects) *in the dev session* of whichever phase needs them —
  never at runtime, never from the phone.
- **One voice.** Design one ElevenLabs voice for the app ("the Voice":
  low, close, unhurried, neither male nor female-coded if possible),
  save it, and record its voice id in this file so every later session
  speaks with the same voice. Spoken lines are rare — silence and text
  stay the default; the voice is an event.
- **Files**: `game3/vigil/audio/<kind>/<name>.ogg` (Ogg Vorbis, mono,
  ~96 kbps, trimmed, loudness-normalised to about −20 LUFS so rites
  don't jump in volume), listed in `data/sounds.json` with the prompt
  that made each one, the tool, the date and its duration — so a sound
  can be regenerated or replaced later. Commit the `.ogg`, not the
  flow/canvas.
- **Recipes reference sounds by id** (`"sounds": {"payoff": "whisper.turn_around"}`),
  so thousands of recipes share a library of perhaps 100–300 sounds;
  a recipe never owns a unique file. Budget: keep all audio under
  ~25 MB for the app download.
- **Licensing:** commercial use of ElevenLabs output depends on the
  account's plan — check it is a paid plan with commercial rights before
  the store release (Phase 8), and note the plan in `data/sounds.json`.
- Every generated sound gets a synth fallback in the engine (so a
  missing/removed file never breaks a rite).
- **Tone library** (user's follow-up: "or a tone lib") — two parts, both
  alternatives to generating everything:
  - **Named tone presets in `data/tones.json`**, played by `Synth`:
    singing bowl, struck bell, glass harmonica, 432 Hz tether, low
    drone, binaural pair, heartbeat thump, shortwave carrier, … each a
    small recipe of partials, envelope, detune, noise and reverb.
    Recipes pick tones by id exactly like sounds (`"tones": {"payoff":
    "bowl.low"}`); a new tone is a JSON entry, no code and no file. This
    covers most chimes, drones and pure tones, so ElevenLabs is kept for
    voice and textured foley. Built in Phase 1 with `Synth`.
  - **Ready-made sound libraries** as a third source for foley and
    ambience: CC0 only (e.g. Freesound filtered to CC0, Kenney audio
    packs) so there is nothing to attribute or license; same `.ogg`
    format and `sounds.json` entry, with `source` + URL instead of a
    prompt. Non-CC0 packs (even "royalty-free") need the licence file
    committed next to them and a check it allows app distribution.
  - A JS tone library (Tone.js) can't run inside Godot; if one is ever
    wanted it's only as an offline tool rendering `.ogg` files.

### Registers: eerie one day, tender the next

(Added from the user's follow-up: "It's cool that some rituals are
eerie or out there while others are loving and nice. Like every day is
different.")

Every recipe carries one `register`:

| register | feel | examples |
|---|---|---|
| `eerie` | uncanny, confronting, disorienting | r01, r03, r04, r07, r13, r14 |
| `stark` | austere, bodily, silent | r02, r11, r16, r20, r17 |
...
tender rites get warm grounds and soft strokes, eerie ones the cold,
glitching ones. The Phase 10 coverage report counts registers too.

### Teachings: rites that leave the user a practice

(Added from the user's follow-up: some rites should "teach a specific
method or ritual that if the user likes it or finds it works they could
start doing it daily or when needed".)

- A **teaching** is a rite that walks the user through a real,
  repeatable practice — something they could do without the phone
  afterwards. It ends with one quiet offer: "This one can be yours.
  Keep it?" Keep → it goes into **the Book** (below). Not keeping is
  fine and says nothing.
- **The Book** (working name; a small closed-book mark on the threshold
  screen, appearing only once the first practice is kept) lists kept
  practices. Any kept practice can be opened **at any time, as often as
  wanted** — it never seals the day and never counts as the day's rite.
  This is the "when needed" half of the idea; the daily draw stays the
  "what awaits you" half. Each practice in the Book also has a plain
  one-screen text version, so it can be done eyes-off / without the
  animation.
- Real traditions are named and credited in one line (where it comes
  from, e.g. "A Brazilian folk custom." / "From the Buddhist practice of
  mettā."), never mocked, never dressed up as ancient when it isn't.
  No health or outcome claims ("this will heal…", "guaranteed").
- Most teachings are one generic engine, **`practice`** (Phase 12): a
  recipe is a list of steps, each `{text, voice?, tone?, seconds,
  advance: "tap" | "hold" | "time" | "type" | "sensor:<gate>", visual}`,
  over a register-appropriate look. That makes teachings cheap to author
  by the hundred in Phase 10 (breath methods, body scan, gratitude,
  5-4-3-2-1 grounding, Hoʻoponopono, the Ignatian examen, candle gazing,
  writing and burning a worry, …). Teachings that need a special
  mechanic (São Longuinho's jumps) get their own engine.
- Free vs paid for the Book is an open question (see Open questions).
...
Registers bend these rules, never break them: `tender` may be warm and
direct ("You are allowed to receive this."), still without praise or
sweetness; `folk` may be light and a little funny (three jumps in the
kitchen is funny, and that's fine); `eerie` and `stark` stay cold.
...
game3/vigil/
  ...
  data/
    ...
    stones.json          Phase 5
    sounds.json          sound manifest: id → file, source (ElevenLabs prompt / CC0 URL), date
    tones.json           named synth tone presets (the tone library)
  audio/                 generated .ogg (voice/, foley/, ambience/, choir/)
...
- **Payoff:** door cracks open with light and a whisper: "Turn around.
  Look at what is behind you in the physical room." Whisper = the
  Voice, generated with ElevenLabs (see "Sound"); stone-grinding door
  foley generated too. Synthesized breathy noise is the fallback.
...
- **Mechanics:** horizontal finger position = tuner frequency; 5 hidden
  ...
  cluster, deep-sea groan — synthesized in `Synth`, or generated
  sound-effect clips (ElevenLabs) where synth sounds too clean; never
  licensed recordings. Each station plays once.
...
### r24 The Four Warmths — `touch` (+`haptics`) — P12 · tender + teaching
Loving-kindness (from the Buddhist practice of mettā), in the user's
four circles: **yourself, a loved one, a friend, your worst enemy**.
- **Setup:** warm dark ground, a small ember of light in the centre,
  a soft low tone (tone library: `bowl.low`); "Put your thumb on the
  light."
- **Mechanics:** four rounds. Each names who ("Yourself." / "Someone
  you love." / "A friend." / "The one you find hardest to love.");
  the user may type a name or just hold. Holding the ember grows it
  while one short phrase is offered to repeat silently ("May you be
  safe. May you be well. May you be at ease."); a slow heartbeat
  haptic; ~40 s each; the ember is carried outward to a new point of
  light for each round. The enemy round is allowed to be hard: it may
  end early with "Even wanting to is enough." Names are never saved
  unless the practice is kept *and* the user chooses to keep them.
- **Payoff:** four lights on the screen, joined by faint lines; "You
  have been warm toward four people today. One of them was you." Then
  the Keep offer.

### r25 São Longuinho — `accel` (+`mic`) — P12 · folk + teaching
The Brazilian folk custom for lost things: you promise São Longuinho
three little jumps (*"São Longuinho, São Longuinho, se eu achar [o
que perdi], dou três pulinhos"*) and, once the thing is found, you pay
with three jumps and shouts of his name. (Check the exact wording with
a Portuguese speaker before shipping; show it in Portuguese with an
English line under it.)
- **Setup:** a warm, slightly crooked folk-art look (a candle, a
  doorway, a small saint's-card frame — drawn, not a real devotional
  image); a plucked tone or a far-off church bell.
- **Mechanics, part 1 — the promise:** "Think of what you have lost."
  The user types it (kept on device, as the open promise). They say
  the promise aloud (mic hears it if permitted; otherwise tap). Then a
  short, genuinely useful finding method: stop and breathe; picture the
  last time you held it; retrace from there; look at eye level, then
  low, then high; check the place it *should* be twice. The rite then
  ends — the promise stays open.
- **Mechanics, part 2 — paying the debt:** the threshold screen shows
  the open promise quietly (also in the Book). "Found it" opens the
  payment: three jumps detected by the accelerometer (free-fall dip then
  impact spike, `Senses.jumps()`), each answered with a bell and a
  haptic; shouts of "São Longuinho!" counted from mic loudness if
  permitted (never required).
- **Payoff:** the saint's card glows, "The debt is paid." and the
  promise closes; the practice is offered for the Book (from then on,
  "Lost something" is a one-tap entry there). Not found after 7 days:
  the promise just fades, no guilt.
...
Needs Phases 0–1; r25's jumps need Phase 3's `Senses` work (jump
detection can be added here if Phase 3 hasn't run). Can go in any time
after that — it doesn't block or depend on the paid tier.

1. `practice` engine (see "Teachings") + its params schema; step
   runner with the advance types; per-step tone/voice cues.
2. The Book screen + `Save` section for kept practices and open
...
- Voice: direction for "the Voice" (see "Sound") — approve it from 2–3
  ElevenLabs voice designs before Phase 3's r09 records any lines.
...
- The Book: free for everyone (a kept practice is the user's own), or
  free up to 3 kept practices with the rest paid? Before Phase 11.
