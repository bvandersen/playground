...
func _draw() -> void:
	...
	for seg in net.segments:
		draw_polyline(seg.points, BALLAST_EDGE, 30.0)
		_round_ends(seg, 15.0, BALLAST_EDGE)
	for seg in net.segments:
		draw_polyline(seg.points, BALLAST, 26.0)
...
func _round_ends(seg: TrackSegment, r: float, col: Color) -> void:
	...
	if node0.ports.size() > 1:
		draw_circle(seg.points[0], r, col)
	if node1.ports.size() > 1:
		draw_circle(seg.points[seg.points.size() - 1], r, col)
...
func _draw_rails(seg: TrackSegment) -> void:
	...
	for rail in [left, right]:
		draw_polyline(rail, RAIL_BASE, 2.8, true)
		draw_polyline(rail, RAIL_TOP, 1.1, true)
...
func _draw_buffer_stop(node: TrackNode) -> void:
	...
	draw_set_transform(p, d.angle())
	draw_rect(Rect2(-4.0, -9.0, 5.0, 18.0), Color(0.2, 0.2, 0.2))
	for k in range(3):
		var y := -9.0 + k * 6.0
		draw_rect(Rect2(-3.5, y + 0.5, 4.0, 3.0), Color(0.95, 0.78, 0.15))
		draw_rect(Rect2(-3.5, y + 3.5, 4.0, 2.5), Color(0.85, 0.2, 0.15))
	draw_circle(Vector2(2.0, -GAUGE * 0.5), 1.8, Color(0.12, 0.12, 0.12))
	draw_circle(Vector2(2.0, GAUGE * 0.5), 1.8, Color(0.12, 0.12, 0.12))
...
func _draw_switch(node: TrackNode) -> void:
	for p in node.ports:
		...
		for q in c:
			...
			if pts.size() >= 2:
				draw_polyline(pts, Color(0, 0, 0, 0.35), 5.0, true)
				draw_polyline(pts, Color(0.35, 0.95, 0.45, 0.95) if on else Color(0.95, 0.3, 0.25, 0.85), 3.0, true)
	draw_circle(node.position, 5.5, Color(0.08, 0.08, 0.1, 0.85))
	draw_arc(node.position, 5.5, 0.0, TAU, 20, Color(1, 1, 1, 0.85), 1.2, true)
	draw_circle(node.position, 2.6, Color(0.35, 0.95, 0.45))
